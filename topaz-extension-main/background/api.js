...
class API {
  ...
  // Save blocked content for authenticated users
  async saveUserBlockedContent(sessionId, blockedItems) {
    console.log(`🔐 API: Saving blocked content for authenticated user`);
    
    // Check if user is authenticated
    if (!this.authState.isAuthenticated || !this.authState.user || !this.authState.accessToken) {
      console.log(`🔐 API: User not authenticated, skipping blocked content save`);
    ...
    try {
      ...
      console.log(`🔐 API: Sending blocked content for user ${this.authState.user.email}`);

      const response = await fetch(`${API_ENDPOINTS.BASE_URL}/api/user-blocked-content`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok) {
        console.error(`🔐 API: Save blocked content failed - HTTP ${response.status}: ${response.statusText}`);
        return { success: false, error: `HTTP ${response.status}` };
      }

      const data = await response.json();
      console.log(`🔐 API: Blocked content saved successfully for user ${this.authState.user.email}`);
      return { success: true, data };
    } catch (error) {
      console.error(`🔐 API: Save blocked content error: ${error.message}`);
      return { success: false, error: error.message };
    }
  }

  // Get blocked content for authenticated users
  async getUserBlockedContent() {
    console.log(`🔐 API: Getting blocked content for authenticated user`);
    
    // Check if user is authenticated
    if (!this.authState.isAuthenticated || !this.authState.user || !this.authState.accessToken) {
      console.log(`🔐 API: User not authenticated, cannot get blocked content`);
      return { success: false, error: 'User not authenticated' };
    }

    try {
      const url = `${API_ENDPOINTS.BASE_URL}/api/user-blocked-content/${this.authState.user.id}?access_token=${this.authState.accessToken}`;
      ...
      if (!response.ok) {
        console.error(`🔐 API: Get blocked content failed - HTTP ${response.status}: ${response.statusText}`);
        return { success: false, error: `HTTP ${response.status}` };
      }

      const data = await response.json();
      console.log(`🔐 API: Retrieved ${data.count} blocked content items for user ${this.authState.user.email}`);
      return { success: true, data };
    } catch (error) {
      console.error(`🔐 API: Get blocked content error: ${error.message}`);
      return { success: false, error: error.message };
    }
  }
