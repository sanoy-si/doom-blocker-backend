...
class ContentPreprocessor:
    ...
    def enhance_grid_text(self, grid_text: str, platform: str) -> str:
        """
        Enhance grid text with platform-specific context
        """
        if not grid_text:
            return ""
        
        # Clean the grid text
        cleaned = self.clean_text(grid_text, platform)
        
        # Add platform context
        if platform == 'youtube':
            return f"[YOUTUBE_GRID] {cleaned}"
        elif platform == 'twitter':
            return f"[TWITTER_GRID] {cleaned}"
        elif platform == 'reddit':
            return f"[REDDIT_GRID] {cleaned}"
        else:
            return f"[GENERIC_GRID] {cleaned}"
